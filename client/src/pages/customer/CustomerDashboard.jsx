...
const CustomerDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [bookingsRes, insuranceRes] = await Promise.all([
          api.get('/customers/bookings'),
          api.get('/insurance/customer')
        ]);
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Insurance Reminder Widget */}
      {insuranceRecords.filter(r => {
        const daysLeft = Math.ceil((new Date(r.expiryDate) - new Date()) / (1000 * 60 * 60 * 24));
        return (daysLeft <= 30 || r.status === 'Expired') && r.status !== 'Policy Renewed' && r.status !== 'Cancelled';
      }).map((record) => {
        const daysLeft = Math.ceil((new Date(record.expiryDate) - new Date()) / (1000 * 60 * 60 * 24));
        ...
        return (
          <motion.div 
            ...
            animate={{ opacity: 1, y: 0 }}
            className={`border rounded-xl p-4 flex flex-col md:flex-row items-center justify-between gap-4 ${isExpired ? 'bg-red-500/10 border-red-500/20' : 'bg-orange-500/10 border-orange-500/20'}`}
          >
            <div className="flex items-center gap-4">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isExpired ? 'bg-red-500/20 text-red-500' : 'bg-orange-500/20 text-orange-500'}`}>
                <ShieldAlert className="w-5 h-5" />
              </div>
              <div>
                <h3 className={`font-bold ${isExpired ? 'text-red-500' : 'text-orange-500'}`}>
                  {isExpired ? 'Insurance Expired!' : 'Insurance Expiring Soon!'}
                </h3>
                <p className="text-gray text-sm">
                  Policy for vehicle <strong>{record.vehicleId?.registrationNumber}</strong> {isExpired ? 'has expired' : `expires in ${daysLeft} days`}.
                </p>
              </div>
            </div>
            <Link 
              to={`/customer/insurance/renew`}
              className={`px-4 py-2 rounded-lg font-bold text-sm whitespace-nowrap transition-colors ${isExpired ? 'bg-red-500 hover:bg-red-600 text-white' : 'bg-orange-500 hover:bg-orange-600 text-white'}`}
            >
              Renew Now
            </Link>
          </motion.div>
