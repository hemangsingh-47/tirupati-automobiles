        ...
        {team.map((member) => (
          <div key={member._id} className="bg-surface border border-white/5 rounded-2xl overflow-hidden shadow-2xl group">
            <div className="aspect-[3/4] relative bg-background border-b border-white/5">
              {member.imageUrl ? (
                <img src={`http://localhost:5000/uploads/${member.imageUrl}`} alt={member.name} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500" />
              ) : (
                ...
                <div className="flex items-center gap-4">
                  {(imageFile || editingMember?.imageUrl) && (
                    <div className="w-16 h-16 rounded-lg bg-background border border-white/10 overflow-hidden shrink-0">
                      <img src={imageFile ? URL.createObjectURL(imageFile) : `http://localhost:5000/uploads/${editingMember.imageUrl}`} alt="Preview" className="w-full h-full object-cover" />
                    </div>
                  )}
